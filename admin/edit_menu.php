...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if (!$nama || !in_array($kategori, ['makanan', 'minuman']) || $harga <= 0) {
            ...
            $gambar = $menu['gambar']; // Keep existing image
            ...
            if (!empty($_FILES['gambar']['name']) && $_FILES['gambar']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!in_array($fileExt, $allowed)) {
                    ...
                    // Hapus gambar lama
                    if ($menu['gambar'] && file_exists(UPLOAD_DIR . $menu['gambar'])) {
            ...
            if (!$error) {
                $stmt = $pdo->prepare("UPDATE menu SET nama_menu = ?, kategori = ?, harga = ?, gambar = ? WHERE id = ?");
                $stmt->execute([$nama, $kategori, $harga, $gambar, $id]);

                $_SESSION['flash_success'] = 'Menu "' . $nama . '" berhasil diperbarui!';
...
$pageTitle = 'Edit Menu - WartegGo Admin';
...
<div class="admin-content">
    <h1 class="section-title" style="margin-top:0">Edit <span>Menu</span></h1>
    ...
    <div style="max-width:500px;">
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" id="nama_menu" name="nama_menu" class="form-input" value="<?= e($menu['nama_menu']) ?>" required>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="harga" name="harga" class="form-input" value="<?= $menu['harga'] ?>" min="1" required>
            </div>

            <div class="form-group">
                <label for="gambar">Ganti Gambar (opsional)</label>
                <?php if (!empty($menu['gambar'])): ?>
                    <p style="font-size:0.8rem;color:var(--text-muted);margin-bottom:6px;">Gambar saat ini: <?= e($menu['gambar']) ?></p>
                <?php endif; ?>
